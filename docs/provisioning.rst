...
Myvoice is deployed on the following stack.
...
You will also need to add the ``BROKER_URL`` to the ``myvoice.settings.production`` so
...
RabbitMQ user will be named myvoice_<environment> and the vhost will be named myvoice_<environment>
