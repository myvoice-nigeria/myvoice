...
for via_operator, _ in test_phones:
    for phone_operator, phone in test_phones:
        ...
        text = ('Test message to %s via %s. Sent at %s WAT.' %
                (phone_operator, via_operator, now))
        params = {
            'username': 'rapidsms',
        ...
        try:
            ...
            result = e
        print ('Status code and result for %s via %s: %s' %
               (phone_operator, via_operator, result.getcode()))
        print result.read()
        print ''
