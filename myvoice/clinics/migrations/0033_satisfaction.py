...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for visit in orm.Visit.objects.all():
            responses = visit.surveyquestionresponse_set.filter(
                question__for_satisfaction=True)
            negative_responses = responses.filter(positive_response=None)
            if responses:
                if negative_responses:
                    visit.satisfied = False
                else:
                    visit.satisfied = True
                visit.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for visit in orm.Visit.objects.all():
            visit.satisfied = None
            visit.save()
