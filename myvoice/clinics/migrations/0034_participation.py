...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for visit in orm.Visit.objects.all():
            responses = visit.surveyquestionresponse_set.all()
            if responses:
                visit.survey_started = True
            last_survey = responses.filter(question__last_required=True)
            if last_survey:
                visit.survey_completed = True
            visit.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        orm.Visit.objects.all().update(survey_started=False, survey_completed=False)
