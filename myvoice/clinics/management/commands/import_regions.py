...
from myvoice.clinics.models import Region
...
class Command(BaseCommand):
    """Download and import Nigeria LGAs"""
    url = "http://biogeo.ucdavis.edu/data/gadm2/shp/NGA_adm.zip"
    ...
    def handle(self, *args, **options):
        ...
        for schema in (Country(), State(), LGA()):
...
class Schema(object):
    ...
    def from_feature(self, feature):
        fields = {
            'name': unicode(feature[getattr(self, 'name')]),
            'type': self.type,
            'external_id': unicode(feature[getattr(self, 'external_id')]),
        ...
        if hasattr(self, 'alternate_name'):
            fields['alternate_name'] = unicode(feature[self.alternate_name])
        return fields
...
class LGA(Schema):
    """Map LGA shapefile to Region model"""
    type = 'lga'
    filename = 'NGA_adm2.shp'
    # field name map:
    name = 'NAME_2'
