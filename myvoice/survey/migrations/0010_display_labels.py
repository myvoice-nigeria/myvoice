...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        labels = ['Open Facility', 'Respectful Staff Treatment',
                  ...
                  'Wait Time']
        for label in labels:
            orm.DisplayLabel.objects.create(name=label)

        for question in orm.SurveyQuestion.objects.all():
            if question.label in labels:
                question.display_label = orm.DisplayLabel.objects.get(name=question.label)
                question.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        orm.SurveyQuestion.objects.update(display_label=None)
        orm.DisplayLabel.objects.all().delete()
